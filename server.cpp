...
DWORD WINAPI ReceiveThread(LPVOID arg) {
    ...
    while (true) {
        ...
        if (bytes_received <= 0) {
            ...
            LogMessage(std::string(clientInfo->name) + " disconnected.");
            break;
        }
...
int main() {
    ...
    for (int i = 0; i < MAX_CLIENT; i++) {
        ...
        CreateThread(NULL, 0, ReceiveThread, (LPVOID)&Clients[i], 0, NULL);
        CreateThread(NULL, 0, SendThread, (LPVOID)&Clients[i], 0, NULL);
    }
